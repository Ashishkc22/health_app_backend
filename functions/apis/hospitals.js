...
router.get('/', async (req, res) => {
    try {
        ...
        if (user == null || (user.status != "Verified")) {
            return res.status(200).json({
                'status': 'failed',
                'message': user == null ? 'Access Denied' : (`${user.status} User: Access Denied`)
            });
        }
        var qry = {};
        if (req.query.q != null) {
            if ((req.query.q.toString().length == 6) && (parseInt(req.query.q.toString()) > 0)) {
        ...
        if (req.query.duration != null) {
            ...
            if (req.query.duration == "TODAY") {
                ...
                qry.created_at = {
                    $gte: (new Date(nowDate.getFullYear(), nowDate.getMonth(), nowDate.getDate(), 0, 0)).getTime()
                };
            }
                ...
                qry.created_at = {
                    $gte: (new Date(nowDate.getFullYear(), nowDate.getMonth(), nowDate.getDate(), 0, 0)).getTime() - (weekDay * 24 * 60 * 60 * 1000)
                };
            }
            else if (req.query.duration == "THIS MONTH") {
                const now = new Date(Date.now());
                qry.created_at = {
                    $gte: parseInt(new Date(now.getFullYear(), now.getMonth(), 1).valueOf())
                };
            } else if (req.query.duration == "ALL") {

            }
            else if (parseInt(req.query.duration) != null && parseInt(req.query.duration) != NaN) {
                var ltDur = (parseInt(req.query.till_duration) != null && parseInt(req.query.till_duration) != NaN) ? parseInt(req.query.till_duration) : (parseInt(req.query.duration) + (24 * 60 * 60 * 1000));
                qry.created_at = {
        ...
        const resp = await hospitalSch.find(qry).skip(parseInt(req.query.page || 0) * parseInt(req.query.limit || "40")).limit(parseInt(req.query.limit || "40"));
...
router.post('/', async (req, res) => {
    try {
        ...
        if (usr == null || (usr.status != "Verified")) {
            return res.status(200).json({
                'status': 'failed',
                'message': usr == null ? 'Access Denied' : (`${usr.status} User: Access Denied`)
            });
        }

        let uuid;
        while (true) {
            var x = (Math.floor(Math.random() * (9999 - 1001 + 1)) + 1001).toString().padStart(4, '0');
            const uid = `${req.body.category.toString().toUpperCase().substring(0, 1)}${x}`
...
router.patch('/:id', async (req, res) => {
    ...
    try {
        ...
        if (userr == null || (userr.status != "Verified")) {
            return res.status(200).json({
                'status': 'failed',
                'message': userr == null ? 'Access Denied' : (`${userr.status} User: Access Denied`)
            });
        }
